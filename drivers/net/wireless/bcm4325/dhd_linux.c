...
static bool deny_dhd_open = false;
...
#ifndef SRCBASE
#define SRCBASE     "drivers/net/wireless/bcm4325"
#endif
...
dhd_open(struct net_device *net)
{
	...
	if(deny_dhd_open){
		DHD_ERROR(("%s: ---------------deny_dhd_open:%d-------RETURN -1--------\n", __FUNCTION__, deny_dhd_open));
		return -1;
	}
...
extern int sdio_intr_enable(dhd_pub_t *dhdp);
int
intr_enable(struct net_device *dev)
{
	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);
	DHD_INTR(("%s: enable SDIO device interrupts\n", __FUNCTION__));
	sdio_intr_enable(&dhd->pub);
	return 1;
}
