...
volatile static bool isPowerModeBusy = false;
volatile static bool status_on = false;
volatile static bool status_off = false;
...
)
{
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0 && isPowerModeBusy == false) {

		WL_TRACE(("%s: DHCP session starts\n", __FUNCTION__));
		isPowerModeBusy = true;
	...
	else if (strnicmp((char *)&powermode_val, "0", strlen("0")) == 0 && isPowerModeBusy == true) {

		WL_TRACE(("%s: DHCP session done\n", __FUNCTION__));
		isPowerModeBusy = false;
		...
		if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0 && isPowerModeBusy == true)
		    printf("BCM: POWERMODE = 1 but isPowerModeBusy is true");
		else if (strnicmp((char *)&powermode_val, "0", strlen("0")) == 0 && isPowerModeBusy == false)
		    printf("BCM: POWERMODE = 0 but isPowerModeBusy is false");
...
bool
wl_wifi_status_check(void)
{
    /*
    * Let bcmsdh_unregister() only works at "Default" and "After On" condition.
    * -----------------------------------------
    * | Process      | status_on | status_off |
    * |---------------------------------------|
    * | Default      | F         | F          |
    * | During On/Off| T         | T          |
    * | After On     | T         | F          |
    ...
    * -----------------------------------------
    */
	if(status_on==false && status_off==false) {
		return true;
	} else if(status_on==false && status_off==true) {
		pr_info("%s: Status is already off!!\n", __FUNCTION__);
		return false;
	} else if(status_on==true && status_off==false) {
		return true;
	} else {
		pr_info("%s: error!\n", __FUNCTION__);
	}
...
EXPORT_SYMBOL(wl_wifi_status_check);
...
int resuming = 0;
extern int intr_enable(struct net_device *dev);
		...
		if (g_onoff == G_WLAN_SET_OFF) {
			if (strnicmp(extra, "START", strlen("START")) != 0) {
				if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
					...
					WL_TRACE(("%s, Received regular START command(SCAN-ACTIVE)\n", __FUNCTION__));
				} else {
